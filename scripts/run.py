...
if __name__ == "__main__":
    (obj_val_from_gurobi, gurobi_run_time), milp = generate_BMDKP(64, 16, seed = 0)
    logger.info(f"gurobi obj: {obj_val_from_gurobi:.3f}, gurobi run time: {gurobi_run_time:.1f}s")
    obj_val_from_xion, var_ass_from_xion = solve(milp, verbose=True)
    logger.info(f"xion obj: {evaluate_milp_at_var_ass(milp, var_ass_from_xion):.3f}, number of cons violations: {compute_number_of_violated_constraints(milp, var_ass_from_xion)}")
